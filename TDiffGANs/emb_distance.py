...
class EmbeddingDetector():
    def __init__(self, train_embs):
        ...
        # knn
        self.clf = NearestNeighbors(n_neighbors=2, metric='cosine')
    ...
    def maha_score(self, test_embs):
        ...
        return score.item() if isinstance(score, np.ndarray) else score
    ...
    def lof_score(self, test_embs):
        score = -self.lof.score_samples(test_embs).sum(-1)
        score = self.delnan(score)
        return score.item() if isinstance(score, np.ndarray) else score
